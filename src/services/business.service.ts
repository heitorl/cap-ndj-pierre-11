...
class BusinessService {
  ...
  registerCollaborator = async (
    collaborator: Collaborators,
    business: Business,
    datas: Partial<Collaborators>
  ) => {
    const repositoryCollaborator = AppDataSource.getRepository(Collaborators);

    const collaboratorDb = await repositoryCollaborator
      .createQueryBuilder("collaborators")
      .innerJoinAndSelect("collaborators.busine", "business")
      .where({ collaboratorId: collaborator.collaboratorId })
      .getOne();
    if (collaboratorDb?.busine) {
      throw new ErrorHandler(409, "Collaborator is already registered");
    }

    await repositoryCollaborator.update(collaborator.collaboratorId, {
      isPaymaster: datas.isPaymaster,
      busine: business,
    });
    return await repositoryCollaborator.findOneBy({
  ...
  updateCollaborator = async (
    ...
    const collaboratorDb = await repositoryCollaborator
      ...
      .getOne();
    if (
      !collaboratorDb?.busine ||
      collaborator.busine.busineId !== business.busineId
    ) {
  ...
  readsCollaborators = async (business: Business) => {
    const repository = AppDataSource.getRepository(Business);
    const businessDb = await repository
      .createQueryBuilder("business")
      .innerJoinAndSelect("business.collaborators", "collaborators")
      .where({ busineId: business.busineId })
      .getOne();
  ...
  readsBusinessCollaborator = async (collaborator: Collaborators) => {
    ...
    const collaboratorDb = await repository
      .createQueryBuilder("collaborators")
      .innerJoinAndSelect("collaborators.busine", "business")
      .where({ collaboratorId: collaborator.collaboratorId })
      .getOne();
